...
- Supabase Storage is used through the custom Django storage backend in `backend/apps/supabase_storage.py`.
- Vault resources already use Django-protected preview and download endpoints:
  - `/api/app/resources/<course_id>/items/<pk>/preview/`
  - `/api/app/resources/<course_id>/items/<pk>/download/`
- Those resource endpoints require Django authentication and check ownership before streaming files with `FileResponse`.
- This means Vault resource preview/download behavior is close to private-bucket ready.
- The app still exposes some direct public Supabase URLs through `file_url`, avatar URLs, and other storage-backed fields.
- Do not make the full Supabase bucket private without updating every direct public file URL path.
- For fully private storage, prefer Django proxy endpoints or signed URLs generated by the backend after permission checks.
- Authentication is Django token based; Supabase `authenticated` storage policies do not automatically recognize these users.
- Render deployment is configured with Django/Daphne, PostgreSQL, Redis, Supabase Storage, Groq/OpenRouter AI keys, and Resend email settings.
