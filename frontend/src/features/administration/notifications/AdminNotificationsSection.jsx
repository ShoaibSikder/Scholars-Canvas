...
export default function AdminNotificationsSection({ notifications, announcement, onAnnouncementChange, onRunAction, actions }) {
  return (
    <div className="grid min-h-[24rem] min-w-0 gap-4 xl:h-[calc(100dvh-12rem)] xl:max-h-[46rem] xl:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
      <Panel title="Send announcement" className="flex min-h-[24rem] min-w-0 flex-col overflow-hidden xl:h-full xl:overflow-hidden">
        <div className="thin-scrollbar grid min-h-0 min-w-0 flex-1 content-start gap-2 overflow-y-auto pr-1">
          ...
          <textarea className={`${adminInput} min-h-28 w-full min-w-0 resize-y py-2 xl:min-h-36`} placeholder="Message" value={announcement.message} onChange={(event) => onAnnouncementChange((current) => ({ ...current, message: event.target.value }))} />
      ...
      <Panel title="Templates and recent announcements" className="flex min-h-[24rem] min-w-0 flex-col overflow-hidden xl:h-full xl:overflow-hidden">
        <div className="min-h-0 flex-1 overflow-hidden">
          <DataTable className="h-full min-h-0 !max-h-none" rows={notifications?.announcements ?? []} columns={[
