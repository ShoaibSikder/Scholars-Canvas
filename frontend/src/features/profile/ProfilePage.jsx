  ...
  return (
    <div className="grid gap-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-base font-black tracking-tight text-slate-950 dark:text-white">Profile</h1>
      ...
      <div className="grid gap-3 xl:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.8fr)]">
        <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={card}>
          <div className="mb-4 flex items-center gap-3 rounded-lg bg-gradient-to-r from-blue-600 to-violet-600 p-3 text-white">
            <div className="grid size-8 place-items-center rounded-lg bg-white/20">
              <User size={34} />
            </div>
            <div>
              <h2 className="text-base font-black">{draft.full_name || "Scholar"}</h2>
              <p className="text-sm font-semibold text-white/80">Student â€¢ {draft.major || "Major not set"}</p>
            </div>
          </div>

          <form id="profile-form" className="grid gap-3" onSubmit={handleSubmit}>
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-2">
            {[
              ...
              <div key={label} className="flex gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-950/50">
        ...
        <div className="grid gap-3 content-start">
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={card}>
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-base font-black text-slate-950 dark:text-white">Progress Snapshot</h2>
              <Sparkles className="size-5 text-slate-500 dark:text-slate-400" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              {stats.map((stat) => (
                <div key={stat.label} className="rounded-lg bg-slate-50 p-3 text-center dark:bg-slate-950/50">
                  <strong className="block text-base text-blue-600 dark:text-blue-300">{stat.value}</strong>
          ...
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={card}>
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-base font-black text-slate-950 dark:text-white">Highlights</h2>
              <Shield className="size-5 text-slate-500 dark:text-slate-400" />
            </div>
            <ul className="grid gap-3">
              {achievements.map((item) => (
                <li key={item} className="rounded-lg bg-emerald-50 px-3 py-1.5 text-sm font-bold text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300">{item}</li>
              ))}
            </ul>
          </motion.section>
        </div>
